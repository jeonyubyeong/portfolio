...
import { DataProps, InformationProps, ProjectProps, universityProps } from "@/types";
...
export const getStaticProps = async () => {
  ...
  const universityWithData = objectData.university.map(
    async (item: universityProps) => {
      return getImgSrc({
        section: "university",
        item: await getMd({ section: "university", item }),
      });
    },
  );
  ...
  return {
    props: {
      ...
      university: await Promise.all(universityWithData),
...
const getMd = async ({
  ...
  section: string;
  item: InformationProps | ProjectProps | universityProps;
}) => {
  try {
